...
class ExerciseViewController: UIViewController, AVAudioRecorderDelegate {
    ...
    fileprivate var questionsArray: [Array<String>] = []
    //fileprivate var translationsArray: [String] = []
    ...
    override func viewDidLoad() {
        ...
        //translationsArray = selectedTranslations!
        
        karaokeLabel.text = questionsArray[questionSelected][0]
        
        translationLabel.text = "\(questionsArray[questionSelected][2])\n\(questionsArray[questionSelected][1])"
        
        characterCount = questionsArray[questionSelected][0].characters.count
    ...
    func crappyBugFix() {
        self.statusView.layer.backgroundColor = UIColor(red: 131/255.0, green: 180/255.0, blue: 219/255.0, alpha: 1.0).cgColor
        self.statusImage.image = UIImage(named: "statusSpeaking.png")
        self.statusLabel.text = "\((selectedVoice?.name)!) is speaking."
...
extension ExerciseViewController: AVSpeechSynthesizerDelegate {
    ...
    func setSpeaker() {
        ...
        speakerImage.image = UIImage(named: "\(selectedImageName).png") ?? UIImage(named: "chinesegirl.png")
        personTalkingLabel.text = (selectedVoice?.name)!
    }
    
    func startSpeaking() {
        if hasRecording == false {
            statusView.layer.backgroundColor = UIColor(red: 131/255.0, green: 180/255.0, blue: 219/255.0, alpha: 1.0).cgColor
            statusImage.image = UIImage(named: "statusSpeaking.png")
            statusLabel.text = "\((selectedVoice?.name)!) is speaking."
        } else {
            statusView.layer.backgroundColor = UIColor(red: 219/255.0, green: 214/255.0, blue: 131/255.0, alpha: 1.0).cgColor
        ...
        let speechString : String = questionsArray[questionSelected][0]
        let nextSpeech:AVSpeechUtterance = AVSpeechUtterance(string: speechString)
        nextSpeech.voice = selectedVoice
        nextSpeech.rate = AVSpeechUtteranceDefaultSpeechRate * 0.75
...
extension ExerciseViewController: AVAudioPlayerDelegate {
    ...
    func startRecording() {
        ...
        statusView.layer.backgroundColor = UIColor(red: 219/255.0, green: 131/255.0, blue: 136/255.0, alpha: 1.0).cgColor
        statusImage.image = UIImage(named: "statusRecording.png")
        statusLabel.text = "Recording. Repeat Line."
        toggleStopwatch()
...
extension ExerciseViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if premiumStatus == nil {
            if 0 ... 4 ~= indexPath.row {
                karaokeLabel.text = questionsArray[indexPath.row][0]
                translationLabel.text = "\(questionsArray[indexPath.row][2])\n\(questionsArray[indexPath.row][1])"
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let question = questionsArray[indexPath.row][0]
        let translation = "\(questionsArray[indexPath.row][2])\n\(questionsArray[indexPath.row][1])"
        ...
        if premiumStatus == nil {
            if 0 ... 4 ~= indexPath.row  {
                ...
                cell.translationLabel.alpha = 1.0
            } else {
                cell.questionLabel.alpha = 0.1
                cell.numberLabel.alpha = 0.1
                cell.translationLabel.alpha = 0.1
            }
        } else {
            cell.questionLabel.alpha = 1.0
            cell.numberLabel.alpha = 1.0
